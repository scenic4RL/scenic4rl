...
class GFScenicEnv(gym.Env):
	...
	def __init__(self, scenario_file, gf_env_settings={}, render = False, compute_scenic_behavior=False, use_default_ai_behavior=False, rank=0):
		...
		self.rank = rank
		self.use_default_ai_behavior = use_default_ai_behavior
		self.compute_scenic_behavior = compute_scenic_behavior

		from scenic.simulators.gfootball.utilities.scenic_helper import buildScenario
		self.scenario = buildScenario(self.scenario_file)
		...
		if not self.compute_scenic_behavior and not self.use_default_ai_behavior:
			assert False, "compute_scenic_behavior flag must be set if use_default_ai_behavior is False"

		assert self.use_default_ai_behavior or self.compute_scenic_behavior, "compute_scenic_behavior flag must be set if use_default_ai_behavior is False"
		...
		for _ in range(100):
			try:
				...
				self.simulation = GFootBallSimulation(scene=self.scene, settings={}, for_gym_env=True,
													  render=self.render, verbosity=1,
													  env_type="v2",
